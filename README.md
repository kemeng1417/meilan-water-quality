...
- **数据填报** — 矩阵模式 / 逐点模式，批量粘贴/清空整列/重置本行，Tab/Enter 键盘导航，60s 自动保存，全屏填报，Undo/Redo
- **即时合规判定** — 录入时本地即时对照标准限值判定合格/超标，绿/红底色标记，列头 × 清空、右键快捷填充
...
- **审核流程** — 化验员填报 → 提交审核 → 审核人通过/打回，结论实时自动生成
- **权限管理** — 主管(admin)可删除已审核报告、管理用户账号；化验员(tester)填报+审核
...
| 用户名 | 密码 | 姓名 | 角色 |
|--------|------|------|------|
| admin | admin123 | 主管 | 主管（全部权限） |
| huayan | 123456 | 化验员 | 化验员 |
| zhengqingshan | 123456 | 郑清山 | 化验员 |
| kemeng | 123456 | 柯猛 | 化验员 |
| lizhenhui | 123456 | 黎振辉 | 化验员 |
| xingzhouyu | 123456 | 邢周玉 | 化验员 |
| wangheshan | 123456 | 王和善 | 化验员 |
| chenying | 123456 | 陈颖 | 化验员 |
| liuyi | 123456 | 刘毅 | 化验员 |
| zhangwei | 123456 | 张伟 | 化验员 |
| bairui | 123456 | 白瑞 | 化验员 |
